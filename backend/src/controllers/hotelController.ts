...
//////////////////////////////////////////////
/*
This is a new API for FrontEnd
*/
///////////////////////////////////////////////
/* 
GET /hotel/:slug/:id
Retrieve hotel details using a combination of slug and ID.
*/
export const getHotelDetails = (req: Request, res: Response): any => {
  const { slug, id } = req.params; // Extract slug and ID from request parameters

  try {
    // Validate parameters
    if (!slug || !id) {
      return res.status(400).json({ message: 'Missing slug or id parameter' });
    }

    // Fetch hotel by ID first
    const filePath = getHotelFilePath(id);

    if (!fs.existsSync(filePath)) {
      return res.status(404).json({ message: 'Hotel not found by ID' });
    }
    ...
    if (hotelData.slug !== slug) {
      return res.status(404).json({ message: 'Hotel not found by slug and ID combination' });
    }

    // Respond with the hotel details
